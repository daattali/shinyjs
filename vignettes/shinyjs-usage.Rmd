...
author: "Dean Attali, Pepijn de Vries"
date: "`r Sys.Date()`"
...
- [Using shinyjs with secure_app](#usage-secureapp)
...
- In Shiny apps that use a `secure_app` (built using the `shinymanager` package)
- In Rmd documents
...
<h2 id="usage-secureapp">Using shinyjs with secure_app layout</h2>

When running a shinymanager secure_app, the user is asked for credentials first, before entering the main Shiny app page. The authorisation page
will not be able to handle `runjs` calls, unless `useShinyjs()` is specifically added to that page as shown in the example below. In the example a
javascript alert message ("hello world") is opened as soon as the server code starts running. When the call to `useShinyjs()` would have been made
inside the fluidpage, the javascript message would only be shown after the user credentials (in the example user name and password are both blank)
are checked.
...
server <- function(input, output, session) {
  auth <- secure_server(check_credentials(data.frame(user = "", password = "", stringsAsFactors = F)))
  
  runjs("alert('hello world');")
}

ui <- secure_app(ui, head_auth = useShinyjs())
